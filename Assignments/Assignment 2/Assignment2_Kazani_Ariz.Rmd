...
```{r}
diamondDS <- diamonds
plot <- ggplot(diamondDS, aes(x=carat, y=price, colour=clarity)) + geom_point() + labs(x="Price", y="Carat", title = "Carat VS Price of Diamonds", colour = "Clarity") 
plot
```
...
```{r}
plot <- plot + geom_smooth(fill=NA, method="loess") + theme_classic()
plot
```
