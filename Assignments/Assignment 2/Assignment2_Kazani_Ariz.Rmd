...
```{r, echo=FALSE}
data("diamonds")
scatPlot <- ggplot(diamonds, aes(x=carat, y=price, colour=clarity)) + geom_point() + labs(x="Carat", y="Price", title = "Carat VS Price of Diamonds", colour = "Clarity") 
scatPlot
```
...
```{r}
scatPlot <- scatPlot + geom_smooth(fill=NA, method="loess") + theme_classic()
scatPlot 
```
...
```{r}
boxPlot <- ggplot(diamonds, aes(x=cut, y=price, colour = cut)) + geom_boxplot(alpha=0.3)  + labs(x="Cut", y="Price", title = "Cut VS Price of Diamonds") 
boxPlot
```
