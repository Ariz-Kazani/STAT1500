...
ggplot(data, aes(x = events, y = pmf)) +
  geom_bar(stat = "identity") +
  geom_line(aes(y = cdf)) +
  labs(title = "Poisson Distribution PMF (bars) and CDF (line)", x = "Number of Events", y = "Probability") +
  theme_classic()
```
...
plot(arr[-100],
     arr[-1],
     main = "Scatter plot of X and X + 1 when using R'srandom function",
...
plot(arr[-100],
     arr[-1],
     main = "Scatter plot of X and X + 1 when using custom random function",
     xlab = "Value at index X",
     ylab = "Value at index X + 1")
```
...
plot(arr1[-100],
     arr1[-1],
     main = "Scatter plot of X and X + 1 when using custom random function M num",
     xlab = "Value at index X",
     ylab = "Value at index X + 1")

plot(arr2[-100],
     arr2[-1],
     main = "Scatter plot of X and X + 1 when using custom random function L num",
     xlab = "Value at index X",
     ylab = "Value at index X + 1")
```
...
simulate_step <- function(probabilities) {
  ...
  returnI <- sample(1:8, size = 1, prob = probabilities)
  
  return(directions[[returnI]])
}
